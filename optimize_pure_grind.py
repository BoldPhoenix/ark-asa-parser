"""
Optimize XP curve using MAXIMUM XP per level
(assumes some players use level boosts, so we want pure grind data)

Theory: Profile XP = actual earned XP
        Level boosts (explorer notes/bosses) give instant levels without adding XP
        So players at same level can have different XP amounts
        We want the HIGHEST XP for each level = pure grind path
"""

# ALL data from both cluster roots
all_data = [
    # PhoenixArk cluster
    ...
    (131, 20947286.0, "Jaxxx", "PhoenixArk/island"),  # Pure grind!
    (131, 2933268.25, "Jaxxx", "PhoenixArk/extinction"),  # Boosted
    ...
    (89, 549540.625, "Anjo", "Amissa"),
    (89, 564968.75, "Lex XD", "Amissa"),
    (90, 424091.96875, "Cassian", "Amissa"),  # Boosted
    (96, 1107271.5, "Mathias", "Amissa"),
    (134, 51915584.0, "LadySif", "Amissa"),
    (143, 142988992.0, "Axl", "Amissa"),
    
    # ark/patreon cluster
    # (132, 4391253.5, "BoldPhoenix", "ark/patreon"),  # EXCLUDED: Clearly boosted (less XP than L131!)
]

print("="*80)
print("XP CURVE OPTIMIZATION - Accounting for Level Boosts")
print("="*80)
print()

# Group by level and find MAX XP (pure grind path)
from collections import defaultdict
level_max_xp = defaultdict(lambda: (0, None, None))

for level, xp, name, server in all_data:
    if xp > level_max_xp[level][0]:
        level_max_xp[level] = (xp, name, server)

pure_grind_data = sorted([(level, xp, name, server) for level, (xp, name, server) in level_max_xp.items()])

print(f"Pure Grind Path (MAX XP per level) - {len(pure_grind_data)} levels:")
print("-"*80)
for level, xp, name, server in pure_grind_data:
    # Find if there were lower XP values for this level
    alternates = [(x, n, s) for l, x, n, s in all_data if l == level and x < xp]
    boost_note = f"  (vs {len(alternates)} boosted)" if alternates else ""
    print(f"L{level:3d}: {xp:>15,.0f} XP | {name:20s} | {server:20s}{boost_note}")
print()
...
# Optimize
print("="*80)
print("Finding optimal multiplier...")
print("="*80)
print()
...
for mult_hundredths in range(1090, 1200):
    ...
    for level, xp, name, server in pure_grind_data:
        ...
        weight = 1.0 if level < 100 else 3.0  # Weight high levels more heavily
        
        total_error += error
        total_weighted_error += error * weight
        predictions.append((level, predicted_level, error, xp, name))
    
    # Use weighted error for optimization
    avg_weighted_error = total_weighted_error / sum(1.0 if l < 100 else 3.0 for l, _, _, _ in pure_grind_data)
...
# Show predictions
print("Predictions for Pure Grind Path:")
print("-"*80)
...
xp_table = {level: calculate_xp_for_level(level, best_multiplier, best_base_xp) for level in range(1, 201)}
...
print(f"\n{'='*80}")
print(f"Based on {len(all_data)} total data points ({len(pure_grind_data)} unique levels)")
print(f"Using MAXIMUM XP per level (pure grind path, no boosts)")
